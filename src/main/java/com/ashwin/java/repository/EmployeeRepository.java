...
public abstract class EmployeeRepository {
    ...
    public Employee editEmployee(Employee employee) {
        if (Objects.isNull(employeeDao().getEmployee(employee.getId()))) {
            throw new WebApplicationException(String.format(EMPLOYEE_NOT_FOUND, employee.getId()), Response.Status.NOT_FOUND);
        }
    ...
    public String deleteEmployee(final int id) {
        ...
        logger.info("Result in EmployeeRepository.deleteEmployee is: {}", result );
        switch (result) {
            case 1:
                return SUCCESS;
            case 0:
